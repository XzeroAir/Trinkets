...
public class BlockTeddyBear extends BlockHorizontal implements IsModelLoaded {
    ...
    public BlockTeddyBear() {
        ...
        this.setRegistryName(Reference.MODID, TrinketsRegistryNames.ModBlocks.TEDDY_BEAR);
    ...
    public BlockTeddyBear(String name) {
        super(TrinketBlockMaterial.ClothTeddyBear);
        this.setSoundType(SoundType.CLOTH);
        this.setRegistryName(Reference.MODID, TrinketsRegistryNames.ModBlocks.TEDDY_BEAR + "_" + name);
        this.setTranslationKey(this.getRegistryName().toString());
        this.setLightLevel(0);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH));
        this.setCreativeTab(CreativeTabs.BUILDING_BLOCKS);
    ...
    public boolean canBlockStay(World world, BlockPos pos, IBlockState state) {
        if (state.getBlock() == this) //Forge: This function is called during world gen and placement, before this block is set, so if we are not 'here' then assume it's the pre-check.
        {
            ...
            return down.isSideSolid(world, pos, EnumFacing.UP);
        }
    ...
    public IBlockState getStateFromMeta(int meta) {
        IBlockState state = this.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(meta));
        return state;
    }
