...
public class Dragon_Food extends FoodBase {

	public Dragon_Food(String name) {
		super(name, 5, 0);
		this.setAlwaysEdible();
		this.setUUID("bf113b39-af17-4b59-abb8-83781b418386");
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (prop != null) {
				if (TrinketsConfig.SERVER.Food.food_effects) {
					if (!(prop.getImbuedRace().equals(EntityRaces.dragon))) {
						prop.setImbuedRace(EntityRaces.dragon);
						prop.sendInformationToAll();
					}
				}
				if (TrinketsConfig.SERVER.Potion.potion_thirst) {
					int amount = 5;
					if ((prop.getCurrentRace().equals(EntityRaces.dragon))) {
						amount = 10;
					}
