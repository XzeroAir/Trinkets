...
public class TrinketsPotionEffect extends PotionEffect {
    ...
    @Override
    public boolean isCurativeItem(@Nonnull ItemStack stack) {
        try {
            final Item item = stack.getItem();
            if (item == Items.POTIONITEM) {
                PotionType type = PotionUtils.getPotionFromItem(stack);
                return ItemStack.areItemStackShareTagsEqual(stack, PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), type));
            }
