...
public class Registries {
    ...
    public static void registerNewRegistry(RegistryEvent.NewRegistry event) {
        ...
        ForgeRegistry<Element> elements = (ForgeRegistry<Element>) elementReg.setType(Element.class)
                ...
                .allowModification().setName(ELEMENTS_ID).create();
        elementRegistry = elements;
        RegistryBuilder<EntityRace> racesReg = new RegistryBuilder<>();
        ForgeRegistry<EntityRace> races = (ForgeRegistry<EntityRace>) racesReg.setType(EntityRace.class).allowModification().setName(RACES_ID).create();
        raceRegistry = races;
        //		RegistryBuilder<EntityRace> racesReg = new RegistryBuilder<>();
    ...
    public static void registerItem(@Nonnull RegistryEvent.Register<Item> event) {
        Trinkets.LOGGER.info("Registering Crafting");
        event.getRegistry().registerAll(ModItems.crafting.ITEMS.toArray(new Item[0]));

        if (TrinketsConfig.SERVER.FOOD.ENABLED) {
            Trinkets.LOGGER.info("Registering Foods");
            event.getRegistry().registerAll(ModItems.foods.ITEMS.toArray(new Item[0]));
        }

        Trinkets.LOGGER.info("Registering Misc");
        ...
        if (Trinkets.MOD_COMPAT.Baubles && !TrinketsConfig.SERVER.GUI.TRINKETS_CONTAINER_ALLOW_BAUBLES) {
            Trinkets.LOGGER.info("Registering Baubles");
            ...
            Trinkets.LOGGER.info("Registering Trinkets");
            ...
            Trinkets.LOGGER.info("Registering Race Trinkets");
    ...
    public static void registerBlock(@Nonnull RegistryEvent.Register<Block> event) {
        Trinkets.LOGGER.info("Registering Flower Blocks");
    ...
    public static void registerPotion(RegistryEvent.Register<PotionType> event) {
        if (TrinketsConfig.SERVER.POTIONS.ENABLED) {
            Trinkets.LOGGER.info("Registering Potions");
