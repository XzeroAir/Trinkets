...
public class AbilityLightningBolt extends Ability implements ITickableAbility, IKeyBindInterface {
    ...
    protected String addCustomDescriptionTags(TranslationHelper helper, String key, int rendMod, int renderID, int compatID) {
        ...
        final TranslationHelper.KeyEntry key1 = new TranslationHelper.LangEntry(langKey, "boltattack", serverConfig.attackAbility);
        final TranslationHelper.KeyEntry key2 = new TranslationHelper.OptionEntry("boltcost", serverConfig.attackAbility, serverConfig.attackCost);
        final TranslationHelper.KeyEntry key3 = new TranslationHelper.OptionEntry("boltdamage", serverConfig.attackAbility, serverConfig.attackDmg);
        final TranslationHelper.KeyEntry key4 = new TranslationHelper.KeyBindEntry("arckb", serverConfig.attackAbility, ModKeyBindings.ARCING_ORB_ABILITY.getDisplayName());
    ...
    protected boolean chargeBolt(EntityLivingBase entity, MagicStats magic, boolean def, boolean aux) {
        ...
        if (!counter.Tick()) {
            if ((entity instanceof EntityPlayer) && entity.world.isRemote) {
                entity.world.playSound((EntityPlayer) entity, entity.getPosition(), SoundEvents.BLOCK_NOTE_BASS, SoundCategory.PLAYERS, 0.5F, (float) StringUtils.getAccurateDouble((Math.min(0.6F + (0.4F * multi), 1F))));
            ...
            if ((entity instanceof EntityPlayer) && entity.world.isRemote) {
                entity.world.playSound((EntityPlayer) entity, entity.getPosition(), SoundEvents.BLOCK_END_GATEWAY_SPAWN, SoundCategory.PLAYERS, 0.5F, 0.5F);
                ScreenOverlayEvents.instance.SyncCost(realCost);
            }
            return false;
        }
