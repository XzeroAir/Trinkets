...
public class AbilityViciousStrike extends AbilityBase implements IAttackAbility {
	...
	public float hurtEntity(EntityLivingBase target, DamageSource source, float dmg) {
		...
		if (source.getTrueSource() instanceof EntityLivingBase) {
			final EntityProperties prop = Capabilities.getEntityRace(source.getTrueSource());
			if ((prop != null)) {
		...
		if (chance == 0) {
			final int duration = (int) (faelis ? serverConfig.duration : serverConfig.duration * 0.33);
