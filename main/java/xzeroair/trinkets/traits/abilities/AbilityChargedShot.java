...
public class AbilityChargedShot extends Ability implements ITickableAbility, IBowAbility, IAttackAbility {
    ...
    public void tickAbility(EntityLivingBase entity) {
        ...
        final boolean bow = entity.isHandActive() && (entity.getActiveItemStack().getItem() instanceof ItemBow) && !blacklistedBow
            ...
            if (!cancel) {
                if ((scale < 1F)) {
                    if (entity.world.isRemote && (entity instanceof EntityPlayer)) {
                        entity.world.playSound((EntityPlayer) entity, entity.getPosition(), SoundEvents.BLOCK_NOTE_GUITAR, SoundCategory.PLAYERS, 0.1F, scale + 0.5F);
                    }
                    critReady = false;
                } else {
                    if (!critReady) {
                        if (entity.world.isRemote && (entity instanceof EntityPlayer)) {
                            entity.world.playSound((EntityPlayer) entity, entity.getPosition(), SoundEvents.EVOCATION_ILLAGER_CAST_SPELL, SoundCategory.PLAYERS, 1F, 0.5F);
                        }
                        critReady = true;
                    }
                }
        ...
        if ((source.damageType.contentEquals("arrow") || (source.isProjectile() && !source.isMagicDamage())) && (damageMultiplier > 0)) {
            ...
            try {
                if (useMP && critReady && serverConfig.explode) {
                    ...
                    NetworkHandler.sendToTracking(new EffectsRenderPacket(attacker, source.getImmediateSource().posX, source.getImmediateSource().posY, source.getImmediateSource().posZ, 0, 0, 0, 0, 5, 0, 0), source.getImmediateSource());
