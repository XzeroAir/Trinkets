...
public class AbilityNullKinetic extends Ability implements IAttackAbility, IJumpAbility {

    float fallMultiplier = 1F;

    public AbilityNullKinetic() {
    ...
    protected String addCustomDescriptionTags(TranslationHelper helper, String key, int rendMod, int renderID, int compatID) {
        final TranslationHelper.KeyEntry key1 = new TranslationHelper.OptionEntry("fdpercent", true, ((100F - (TrinketsConfig.SERVER.Items.INERTIA_NULL.falldamage_amount * 100F)) + "%"));
        return helper.formatAddVariable(key, renderID, key1);
    }

    public AbilityNullKinetic setFallMultiplier(float fallMultiplier) {
