...
public class AbilityResistance extends Ability implements ITickableAbility, IAttackAbility, IPotionAbility {
    ...
    protected String addCustomDescriptionTags(TranslationHelper helper, String key, int rendMod, int renderID, int compatID) {
        ...
        if (compatID == EnumModCompat.EnhancedVisuals.getId()) {
            final boolean evEnabled = Trinkets.MOD_COMPAT.EnhancedVisuals && TrinketsConfig.getClientStore().MOD_COMPAT_ENHANCED_VISUALS && serverConfig.explosion_resist;
            return evEnabled ? super.addCustomDescriptionTags(helper, key, rendMod, renderID, compatID) : "";
        }
        final TranslationHelper.KeyEntry key1 = new TranslationHelper.LangEntry(langKey, "explosionresist", serverConfig.explosion_resist);
        final TranslationHelper.KeyEntry key2 = new TranslationHelper.OptionEntry("explosionresistamount", serverConfig.explosion_resist, ((100F - (serverConfig.explosion_amount * 100F)) + "%"));
        final TranslationHelper.KeyEntry key3 = new TranslationHelper.LangEntry(langKey, "damageignored", serverConfig.damage_ignore);
        final TranslationHelper.KeyEntry key4 = new TranslationHelper.OptionEntry("hitcount", serverConfig.damage_ignore, hitCount);
        final TranslationHelper.KeyEntry key5 = new TranslationHelper.OptionEntry("cfghitcount", serverConfig.damage_ignore, serverConfig.hits);
        String effect = "ERROR";
        try {
