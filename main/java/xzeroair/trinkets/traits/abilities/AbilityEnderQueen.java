...
public class AbilityEnderQueen extends Ability implements ITickableAbility, IPotionAbility, IAttackAbility, IKeyBindInterface {
    ...
    protected String addCustomDescriptionTags(TranslationHelper helper, String langOutput, int rendMod, int renderID, int compatID) {
        ...
        final KeyEntry key1 = new LangEntry(langKey, "damageignored", serverConfig.dmgChance);
        final KeyEntry key2 = new LangEntry(langKey, "endermenchance", serverConfig.spawnChance);
        final KeyEntry key3 = new LangEntry(langKey, "endermenfollow", serverConfig.Follow);
        final KeyEntry key4 = new LangEntry(langKey, "waterhurts", serverConfig.water_hurts);
        final KeyEntry key5 = new OptionEntry("chance", serverConfig.dmgChance || serverConfig.spawnChance, MathHelper.clamp((1F / serverConfig.chance) * 100, Integer.MIN_VALUE, Integer.MAX_VALUE) + "%");
        final KeyEntry key6 = new OptionEntry("tpchance", serverConfig.teleportOnHurt, MathHelper.clamp((1F / serverConfig.teleportChance) * 100, Integer.MIN_VALUE, Integer.MAX_VALUE) + "%");
        final KeyEntry key7 = new OptionEntry("tpcost", serverConfig.teleportOnHurt, "5.0");
        return helper.formatAddVariables(langOutput, renderID, key1, key2, key3, key4, key5, key6, key7);
    }
