...
public class EnderMoveAI extends EntityAIFollow {
	...
	public EnderMoveAI(final EntityEnderman entity) {
		...
		enderman = entity;
		followPredicate = (@Nullable EntityLiving following) -> (following != null) && (enderman.getClass() != following.getClass());
		speedModifier = 1;
			...
			if (!list.isEmpty()) {
				for (final EntityLivingBase entityliving : list) {
					if (entityliving instanceof EntityPlayer) {
						...
						final boolean ability = TrinketHelper.entityHasAbility(Abilities.enderQueen.toString(), entityliving);
