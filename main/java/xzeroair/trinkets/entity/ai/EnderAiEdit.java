...
public class EnderAiEdit extends EntityAINearestAttackableTarget<EntityPlayer> {
	...
	private boolean shouldAttackPlayer(EntityPlayer player) {
		...
		final boolean ability = TrinketHelper.entityHasAbility(Abilities.enderQueen.toString(), player);
		if ((itemstack.getItem() == ModItems.trinkets.TrinketEnderTiara) || (ability) || (tiara == true) || (eye == true) || (itemstack.getItem() == Item.getItemFromBlock(Blocks.PUMPKIN))) {
			return false;
		} else {
	...
	public boolean shouldExecute() {
		...
		player = enderman.world.getNearestAttackablePlayer(enderman.posX, enderman.posY, enderman.posZ, d0, d0, (Function) null, (@Nullable EntityPlayer p_apply_1_) -> (p_apply_1_ != null) && EnderAiEdit.this.shouldAttackPlayer(p_apply_1_));
		return player != null;
	}
