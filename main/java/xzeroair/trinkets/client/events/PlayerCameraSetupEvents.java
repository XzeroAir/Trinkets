...
public class PlayerCameraSetupEvents {
    ...
    private void applyBobbing(float partialTicks) {
        if (mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            GlStateManager.translate(MathHelper.sin(f1 * (float) Math.PI) * f2 * 0.15F, -Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2) * 0.0F, 0.0F);
    ...
    public void CameraSetup(CameraSetup event) {
        if (!TrinketsConfig.CLIENT.CAMERA_HEIGHT || TrinketsConfig.compat.ARTEMIS_LIB) {
            return;
        }
    ...
    public void renderFogDensityEvent(FogDensity event) {
        ...
        if (player.isInWater()) {
            boolean hasAffinity = TrinketHelper.entityHasAbility(player, TrinketsRegistryNames.ModAbilities.AFFINITY_WATER);
            if (hasAffinity) {
    ...
    public void renderBlockOverlay(RenderBlockOverlayEvent event) {
        ...
        if ((overlay == OverlayType.FIRE) || (overlay == OverlayType.WATER)) {
            final boolean cancel = !TrinketHelper.getAccessory(event.getPlayer(), acc -> {
                if ((overlay == OverlayType.FIRE) && (((acc.getItem() instanceof TrinketDragonsEye) && (((TrinketDragonsEye) acc.getItem()).getPrimaryElement() == Elements.FIRE)) || acc.getItem().getRegistryName().compareTo(ModItems.RaceTrinkets.TrinketDragonRing.getRegistryName()) == 0)) {
                    return true;
            ...
            boolean hasFireAffinity = (overlay == OverlayType.FIRE) && TrinketHelper.entityHasAbility(event.getPlayer(), TrinketsRegistryNames.ModAbilities.IMMUNITY_FIRE);
            boolean hasWaterAffinity = (overlay == OverlayType.WATER) && TrinketHelper.entityHasAbility(event.getPlayer(), TrinketsRegistryNames.ModAbilities.AFFINITY_WATER);
