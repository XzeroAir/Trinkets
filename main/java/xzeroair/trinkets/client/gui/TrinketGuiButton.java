...
public class TrinketGuiButton extends GuiButton {
	...
	public TrinketGuiButton(int buttonId, GuiContainer parentGui, int x, int y, int width, int height, String buttonText) {
	...
	{
		if (visible && !mc.getMinecraft().getMinecraft().player.getRecipeBook().isGuiOpen())
		{
			...
			if(id == 9999) {
				...
				TrinketDrawButton(x, y, 16, 16, 16, 16, 0.0625f, 1, 1, 1, 1);
			}
			...
			if(id == ID) {
				...
				final boolean flag = TrinketsConfig.CLIENT.GUI.button.Texture.openButtonTex.equalsIgnoreCase(Reference.RESOURCE_PREFIX + "textures/gui/gui_inventory.png");
				final boolean flag2 = TrinketsConfig.CLIENT.GUI.button.Texture.closeButtonTex.equalsIgnoreCase(Reference.RESOURCE_PREFIX + "textures/gui/gui_inventory.png");
				final float[] obColor = ColorHelper.getHexColor(TrinketsConfig.CLIENT.GUI.button.Texture.openButtonColor);
				...
				final float[] cbColor = ColorHelper.getHexColor(TrinketsConfig.CLIENT.GUI.button.Texture.closeButtonColor);
				...
				if (k==1) {
					if(parentGui instanceof TrinketGui) {
						...
						if(flag2) {
							TrinketDrawButton(tXpos, tYpos, cbX, cbY, cbSize, cbSize, closeScale, cr, cg, cb, ca);
						} else {
							TrinketDrawButton(tXpos, tYpos, cbX, cbY, cbSize, cbSize, closeScale, cr, cg, cb, ca);
						}
					} else {
						...
						if(flag) {
							TrinketDrawButton(tXpos, tYpos, obX, obY, obSize, obSize, openScale, or, og, ob, oa);
						} else {
							TrinketDrawButton(tXpos, tYpos, obX, obY, obSize, obSize, openScale, or, og, ob, oa);
						}
					}
	...
	private void TrinketDrawButton(int x, int y, int texX, int texY, int width, int height, float scale, float r, float g, float b, float a) {
		GlStateManager.pushMatrix();
