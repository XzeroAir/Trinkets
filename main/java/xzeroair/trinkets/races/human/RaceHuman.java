...
public class RaceHuman extends EntityRacePropertiesHandler {
    ...
    private final HumanConfig CONFIG = TrinketsConfig.SERVER.RACES.HUMAN;

    public RaceHuman(@Nonnull EntityLivingBase e, @Nonnull EntityProperties properties) {
        super(e, properties, new RaceCache(EntityRaces.human));
    }

    public RaceHuman(@Nonnull EntityLivingBase e, @Nonnull EntityProperties properties, @Nonnull RaceCache raceCache) {
    ...
    public IRenderRaceHandler getRaceRenderer() {
        if (this.RendererRace == null) {
            this.RendererRace = new RaceHumanRenderer(this.getEntity(), this);
        }
