...
public class RaceGoblin extends EntityRacePropertiesHandler {
    ...
    private final GoblinConfig CONFIG = TrinketsConfig.SERVER.RACES.GOBLIN;

    public RaceGoblin(@Nonnull EntityLivingBase e, @Nonnull EntityProperties properties) {
        super(e, properties, new RaceCache(EntityRaces.goblin));
    }

    public RaceGoblin(@Nonnull EntityLivingBase e, @Nonnull EntityProperties properties, @Nonnull RaceCache raceCache) {
    ...
    public void targetedByEnemy(EntityLivingBase enemy) {
        if (this.CONFIG.FRIENDLY_CREEPERS) {
    ...
    public float isHurt(DamageSource source, float dmg) {
        dmg *= DamageTypeConfigParser.parseDamageTypeConfig(1, source, dmg, this.raceCache.getPrimaryElement(), this.CONFIG.DAMAGE_TYPES_TO_IGNORE).getSecond();
        if (this.CONFIG.NATURAL_RESISTANCE_TO_EXPLOSIVES) {
            if (source.isExplosion()) {
//                if (!TrinketHelper.AccessoryCheck(entity, ModItems.trinkets.TrinketDamageShield)) {
                final float maxHP = this.getEntity().getHealth();
...
//                }
            ...
            if (source.isFireDamage()) {
                float modifier = 0.8F;
                if (source.getDamageType().equals(DamageSource.LAVA.getDamageType())) {
                    modifier = 0.5f;
                }
                return dmg * modifier;
            }
    ...
    public boolean attackedEntity(EntityLivingBase target, DamageSource source, float dmg) {
        ...
        if (this.CONFIG.CREEPERS_EXPLODE_ON_CONTACT) {
            if (attack) {
    ...
    public IRenderRaceHandler getRaceRenderer() {
        if (this.RendererRace == null) {
            this.RendererRace = new RaceGoblinRenderer(this.getEntity(), this);
        }
