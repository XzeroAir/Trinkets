...
public class CombatHandler extends EventBaseHandler {
    ...
    public void ArrowImpactEvent(ProjectileImpactEvent.Arrow event) {
        if (event.getArrow().shootingEntity instanceof EntityLivingBase) {
            Capabilities.getEntityProperties(event.getArrow().shootingEntity, prop -> {
                // Handle Entity Abilities
                Map<String, AbilityHolder> abilities = prop.getAbilityHandler().getActiveAbilities();
                for (Entry<String, AbilityHolder> entry : abilities.entrySet()) {
                    String key = entry.getKey();
...
//        if (true) {
//            StringUtils.sendStatusMessageToPlayer(attacked, attacked.getName() + "|" + source.getDamageType() + "|" + damage, true);
//        }

        if (source.getTrueSource() instanceof EntityLivingBase) {
            EntityLivingBase attacker = (EntityLivingBase) source.getTrueSource();
            // Handle Attacker
            damage = Capabilities.getEntityProperties(attacker, damage, (prop, dmg) -> {
        ...
        // Handle Attacked
        if (damage > 0) {
            damage = Capabilities.getEntityProperties(attacked, damage, (prop, dmg) -> {
